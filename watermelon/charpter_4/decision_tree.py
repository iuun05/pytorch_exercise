...
from utils import entropy, purity, majority, \
...
class C45decisionTree:
    ...
    def __init__(self,
        ...
        assert (maxFeatures is None) or (
                    type(maxFeatures) == int and maxFeatures > 0), '最大特征数maxFeatures应为正整数，或不给定maxFeatures'
...
if __name__ == '__main__':
    ...
    for x_, y in zip(Xtest__, ytest_):
        print(f'预测新样本 {x_} 的类别为 "{y}"')  # 显示预测结果# C45决策树分类.py
from typing import Optional, Sequence
from numpy import ndarray, array, unique, log2, inf
from numpy.random import choice
